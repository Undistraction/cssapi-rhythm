  ...
  describe(`with invalid config param names`, () => {
    it(`throws`, () => {
      ...
      expect(() => createRhythm(value)).toThrowMultiline(`
        [cssapi-rhythm] configure() Arguments included invalid value(s)
          – config: Object included key(s) not on whitelist: ['rootFontSize', 'rhythm', 'horizontalRhythm', 'verticalRhythm', 'renderUnit', 'opticalAdjustment']`)
    })
  })
      ...
      expect(() => createRhythm(value)).toThrowMultiline(`
        [cssapi-rhythm] configure() Arguments included invalid value(s)
          – config: Object included invalid value(s)
            – renderUnit: Value wasn't on the whitelist: ['rem', 'em', 'px']
            – rhythm: Wasn't Valid Number or Wasn't number with unit: 'px'`)
    })
  })
  ...
  describe(`combinations of 'rhythm', 'horizontalRhythm', 'verticalRhythm' and 'opticalAdjustment'`, () => {
    describe(`with only 'horizontalRhythm' set`, () => {
      it(`throws`, () => {
        ...
        [cssapi-rhythm] configure() Arguments included invalid value(s)
          – config: Object must include either a 'rhythm' or both 'hRhythm' and 'vRhythm' value`)
      })
    })
